...
class ProfessionalDeliverableBundle(TimestampMixin, Base):
    ...
    is_degraded: Mapped[bool] = mapped_column(Boolean, default=False)
    degraded_reasons_json: Mapped[list] = mapped_column(JSON, default=list)
    failed_gates_json: Mapped[list] = mapped_column(JSON, default=list)
    warnings_json: Mapped[list] = mapped_column(JSON, default=list)
    missing_artifacts_json: Mapped[list] = mapped_column(JSON, default=list)
    user_message: Mapped[str | None] = mapped_column(String(500), nullable=True)
